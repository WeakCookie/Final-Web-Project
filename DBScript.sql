...
GO
-----
CREATE or ALTER PROCEDURE [sp_InsertProductInfo]
    @name nvarchar(max) = '...',
    @price FLOAT = 0,
    @provider NVARCHAR(max) = '...',
    @short_desc NVARCHAR(max) = '...',
    @long_desc NVARCHAR(max) = '...',
    @active BIT = 1,
    @order_id INT = '',
...
BEGIN
    ...
    VALUES(@name, @price, GETDATE(), @provider, @short_desc, @long_desc, @active, @order_id, @image_path)
...
BEGIN
    ...
    if EXISTS (Select *
        ...
        ELSE
            THROW 51000, 'Product ID does not exist!', 1
END
go
-----
...
BEGIN
    ...
    if EXISTS (Select *
        ...
        ELSE
            THROW 51000, 'Category ID does not exist!', 1
END
go
-----
...
CREATE OR ALTER FUNCTION [sf_Login] (@username nvarchar(MAX), @password nvarchar(max))
RETURNS TABLE
AS
return (
    SELECT * from Users u
...
-- EXEC sp_InsertProductInfo 'Airpod_2', 399, 'Apple', 'Duplicate testing item', 'This is a bluetooth earphone of Apple', 1, 8, ''
-- EXEC sp_UpdateProduct 8, 'Airpod_2_Edited', 599, 'Appleeeeee', 'Duplicate testing item edited name', 'This is a bluetooth earphone of Apple sdfgsfdg', 1, 8, ''
-- EXEC sp_DeleteProduct 8

-- EXEC sp_GetCategoryById 2
-- EXEC sp_InsertCategoryInfo 'Tablet_2'
-- EXEC sp_UpdateCategory 100, 'Smart Phone_Edited'
-- EXEC sp_DeleteCategory 6

-- Call user-defined functions
-- select * from sf_Login ('duymai', '123456')
